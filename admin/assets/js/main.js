...
$(document).ready(function () {
    ...
    var pageStackDash = JSON.parse(localStorage.getItem("pageStackDash")) || [];
    var currentPage = localStorage.getItem("currentPage") || "dashboard.php";
    ...
    function loadContentFromUrl() {
        var pageParam = getUrlParameter("page") || "dashboard.php"; // Set default to dashboard.php
        currentPage = pageParam;
        localStorage.setItem("currentPage", currentPage);
        loadPage(currentPage);
    }
    ...
    function loadPage(pageUrl) {
        $("#content")
            ...
            .fadeOut(150, function () {
                $(this).load("pages/" + pageUrl, function (response, status) {
                    if (status === "error") {
                        $(this).html("<p class='text-danger'>Page not found. Redirecting...</p>").fadeIn(200);
                        setTimeout(() => {
                            loadPage("dashboard.php");
                            updateUrl("page", "dashboard.php");
                        }, 1000); // Redirect after 1 second if page is not found
                    } else {
                        $(this).fadeIn(200);
                    }
                });
